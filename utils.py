...
def get_caption_model_processor(model_name_or_path):
    if model_name_or_path == "florence2":
        model_name_or_path = "microsoft/florence-2-base-ft"
        
    processor = AutoProcessor.from_pretrained(model_name_or_path, trust_remote_code=True)
    
    # For Florence-2, we need to ensure the vision config is set correctly
    if "florence-2" in model_name_or_path.lower():
